...
RSpec.describe WaterDrop::Contracts::MessageOptions do
  ...
  context 'when message_options are valid' do
    it { expect(contract.call(message_options)).to be_success }
  end
  ...
  context 'when we run topic validations' do
    context 'when topic is nil but present in options' do
      ...
      it { expect(contract.call(message_options)).not_to be_success }
    end
