  ...
  module Aspects
    ...
    class AroundAspect < BaseAspect
      around options[:method], interception_arg: true do |interception, proxy, *args, &block|
        ...
        ::WaterDrop.logger.debug("AroundAspect before_message was applied before #{self.class.name} class")
        interception.aspect.handle(self, options, args, options[:before_message])
        result = proxy.call(*args, &block)
        ::WaterDrop.logger.debug("AroundAspect after_message was applied after #{self.class.name} class")
